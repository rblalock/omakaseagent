...
- **The Deslop Critic** — when the dominant failure mode is generic AI phrasing, synthetic voice, unnecessary complexity, an explicit humanize/voice-match request, a detect-only prose audit, or a technical-clarity request. **Prose:** throat-clearing, binary contrasts, false agency, jargon/hedging stacks, meta-commentary, content inflation, and voice matching under the semantic lock (`reference/prose.md`; conditional catalog in `reference/prose-patterns.md`). Detect-only audits quote the exact instance, name the pattern, and give a terse fix without scoring, rewriting, or inferring AI authorship; their narrow output contract overrides normal critique ceremony except the Internal Critique Pass. Do not narrate delegation or wrap that output in a report. For simple technical talk, load STE-inspired technical clarity by default and return the shortest sufficient answer. Use neutral Technical clarity only when the user explicitly opts out of ASD-STE100 guidance. **Code:** unnecessary comments, defensive code, over-explanation, defensive abstractions, "for future flexibility" bloat. Use for pervasive low-value removal in either or both modes.
