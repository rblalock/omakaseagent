...
- **The Deslop Critic** — when the dominant failure mode is generic AI phrasing, synthetic voice, unnecessary complexity, or an explicit humanize/voice-match request. **Prose:** throat-clearing, binary contrasts, false agency, jargon/hedging stacks, meta-commentary, content inflation, and voice matching under the semantic lock (`reference/prose.md`; conditional catalog in `reference/prose-patterns.md`). **Code:** unnecessary comments, defensive code, over-explanation, defensive abstractions, "for future flexibility" bloat. Use for pervasive low-value removal in either or both modes.
