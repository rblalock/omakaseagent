...
- **Control-flow audit.** For branch-heavy hotspots, repeated case enumeration, analyzer hotspots, or numeric simplification claims, use the repository's existing analyzer when present. Compare baseline and treatment with the same tool, version, configuration, and code scope; report the raw delta as evidence, not a verdict. Name the invariant and seek a model or boundary that derives cases instead of enumerating them, then preserve behavior with focused behavior-boundary tests. Moving branches into helpers, dense Boolean expressions, dispatch, or metaprogramming is score gaming unless it simplifies the model. Never add a universal threshold or CI complexity gate merely to satisfy Omakase. Keep necessary validation, permission, protocol, parser, and state-machine decisions explicit; name and test the invariant instead of deleting real domain behavior to lower a score.
