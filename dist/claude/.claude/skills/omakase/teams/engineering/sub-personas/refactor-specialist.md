...
- **Audit control-flow claims.** For branch-heavy or analyzer-identified hotspots, use the repository's existing analyzer when present. Compare baseline and treatment with the same tool, version, configuration, and code scope; report the raw delta as evidence, not a verdict. Name the invariant and seek a model or boundary that derives cases instead of enumerating them, then preserve behavior with focused behavior-boundary tests. Moving branches into helpers, dense Boolean expressions, dispatch, or metaprogramming is score gaming unless it simplifies the model. Never add a universal threshold or CI complexity gate merely to satisfy Omakase. Keep necessary validation, permission, protocol, parser, and state-machine decisions explicit; name and test the invariant instead of deleting real domain behavior to lower a score.
...
2. Identify the highest-leverage simplifications (structural deletion, boundary changes, state model reframing — not cosmetic). Apply the control-flow audit above when it triggers.
